...
```{r, }
# Load data
load(here("analysis/pilot_v2/data/survey_data.Rda"))
load(here("analysis/pilot_v2/data/tweet_data.Rda"))
load(here("analysis/pilot_v2/data/prescreen.Rda"))
```

# Elite Content
```{r }
source(here("analysis/pilot_v2/data/accounts.R"))

library(fuzzyjoin)
# elites <- read.csv(here("analysis/data/Wojcieszak_2022/actors-ideo-scores.csv"))
elites <- data.frame(actor = tolower(accounts), elite = 1)

df <- df %>%
    mutate(author = tolower(author)) %>%
    left_join(elites, by = c("author" = "actor")) %>%
    mutate(elite = ifelse(is.na(elite), 0, 1))
df %>%
    group_by(workerID) %>%
    summarize(elite_count = sum(elite)) %>%
